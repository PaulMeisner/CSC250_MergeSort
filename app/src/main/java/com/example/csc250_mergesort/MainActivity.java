...
public class MainActivity extends AppCompatActivity
{
    ...
    private void mergeSort(ArrayList<String> theList, int begin, int end)
    {
        ...
        if(end != begin)//is the list a one list or not?
        {
           // need to begins and ends
            int firstHalfBegin = begin;
            int secondHalfEnd = end;
            int firstHalfEnd = (end + begin)/2; //finding the middle of the array, end of first list
            int secondHalfBegin = firstHalfEnd + 1; //finding the begin of the second list
            mergeSort(theList, firstHalfBegin, firstHalfEnd); //calling merge sort for first half
            mergeSort(theList, secondHalfBegin, secondHalfEnd); //calling merge sort for second half
        }
